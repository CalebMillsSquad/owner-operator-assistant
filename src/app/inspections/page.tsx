      ...
      <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
        ...
        <section className="space-y-4">
          {inspections.length === 0 ? (
            <div className="panel p-5 text-sm text-slate-500">No inspections on file yet.</div>
          ) : (
              ...
              return (
                <div key={inspection.id} className="panel p-5">
                  <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <h2 className="font-semibold">{inspection.type.replaceAll("_", "-")}</h2>
                  ...
                  {inspection.notes ? <p className="mt-3 text-sm text-slate-300">{inspection.notes}</p> : null}
                </div>
              );
            })
          )}
